...
Shader shaderMulLightingActividad;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	shaderMulLightingActividad.initialize("../Shaders/iluminacion_texture_res.vs", "../Shaders/multipleLightsActividad.fs");
	...
	boxMultTexture.init();
	boxMultTexture.setShader(&shaderMulLightingActividad);
...
void applicationLoop() {
	...
	while (psi) {
		...
		shaderMulLightingActividad.setMatrix4("projection", 1, false,
			glm::value_ptr(projection));
		shaderMulLightingActividad.setMatrix4("view", 1, false,
			glm::value_ptr(view));

		shaderMulLightingActividad.setVectorFloat3("viewPos", glm::value_ptr(camera->getPosition()));
		shaderMulLightingActividad.setVectorFloat3("directionalLight.light.ambient", glm::value_ptr(glm::vec3(0.3, 0.3, 0.3)));
		shaderMulLightingActividad.setVectorFloat3("directionalLight.light.diffuse", glm::value_ptr(glm::vec3(0.7, 0.7, 0.7)));
		shaderMulLightingActividad.setVectorFloat3("directionalLight.light.specular", glm::value_ptr(glm::vec3(0.9, 0.9, 0.9)));
		shaderMulLightingActividad.setVectorFloat3("directionalLight.direction", glm::value_ptr(glm::vec3(-1.0, 0.0, 0.0)));

		shaderMulLightingActividad.setInt("spotLightCount", 0);

		shaderMulLightingActividad.setInt("pointLightCount", 0);

		shaderMulLightingActividad.setVectorFloat2("scaleUV", glm::value_ptr(glm::vec2(1, 1)));

		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textureWaterID);
		shaderMulLightingActividad.setInt("texture1", 0);

		glActiveTexture(GL_TEXTURE0 + 3);
		glBindTexture(GL_TEXTURE_2D, textureSpongeID);
		shaderMulLightingActividad.setInt("texture2", 3);

		shaderMulLightingActividad.setFloat("porcentaje", 0.2);

		glm::mat4 modelActividad = glm::mat4(1.0);

		boxMultTexture.setScale(glm::vec3(10.0, 10.0, 10.0));
		boxMultTexture.setPosition(glm::vec3(0.0, 10.0, 0.0));
		boxMultTexture.render(modelActividad);
